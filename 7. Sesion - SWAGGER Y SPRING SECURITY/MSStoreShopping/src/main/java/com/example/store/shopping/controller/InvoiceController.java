...
public class InvoiceController {
	...
	public ResponseEntity<Invoice> createInvoice(@Valid @RequestBody Invoice invoice, BindingResult result) {
		...
		List<String> productsWithZeroStock = new ArrayList<String>();
		invoice.getItems().forEach(items -> {
			invoiceService.validateStockAndReturnProduct(productsWithZeroStock, items.getProductId());
		});

		if (!productsWithZeroStock.isEmpty()) {
			String message = "No hay stock en los productos: "
					+ productsWithZeroStock.stream().collect(Collectors.joining(", "));
